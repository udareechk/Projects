 ...
 *  Name            :   haar.cpp
 *
 *  Author          :   Francesco Comaschi (f.comaschi@tue.nl); Kesara Gamlath (kesaradhanushka@gmail.com)
 *
 ...
 *		06-12-16 	: 	CUDA Functions added version
 *
 *
...
void ScaleImage_Invoker( myCascade* _cascade, float _factor, int sum_row, int sum_col, std::vector<MyRect>& _vec)
{
	...
	// Malloc Device Memory for all given data 
	...
	// Copy all required data into the Device
	...
	cudaFree(stages_array_cuda); checkCudaError();

	cudaFree(cascade_pq0_cuda); checkCudaError();
	cudaFree(cascade_p0_cuda); checkCudaError();

	// Add collected results to the vector
	for( x = 0; x < x2; x += step ) {
