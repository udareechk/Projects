...
__global__ void runCascadeClassifier(int* result_cuda, int start_stage, int cascade_n_stages, int cascade_inv_window_area, int cascade_sum_width, int cascade_sqsum_width, 
	...
	for( i = start_stage; i < cascade_n_stages; i++ )
		{
			...
			for( j = 0; j < stages_array[i]; j++ )
			{
				...
				int sum = (*(scaled_rectangles_array_cuda[r_index] + p_offset)
						 - *(scaled_rectangles_array_cuda[r_index + 1] + p_offset)
						 - *(scaled_rectangles_array_cuda[r_index + 2] + p_offset)
						 + *(scaled_rectangles_array_cuda[r_index + 3] + p_offset))
					* weights_array[w_index];


				sum += (*(scaled_rectangles_array_cuda[r_index+4] + p_offset)
					- *(scaled_rectangles_array_cuda[r_index + 5] + p_offset)
					- *(scaled_rectangles_array_cuda[r_index + 6] + p_offset)
					+ *(scaled_rectangles_array_cuda[r_index + 7] + p_offset))
					* weights_array[w_index + 1];

				if ((scaled_rectangles_array_cuda[r_index+8] != NULL))
					sum += (*(scaled_rectangles_array_cuda[r_index+8] + p_offset)
						- *(scaled_rectangles_array_cuda[r_index + 9] + p_offset)
						- *(scaled_rectangles_array_cuda[r_index + 10] + p_offset)
						+ *(scaled_rectangles_array_cuda[r_index + 11] + p_offset))
						* weights_array[w_index + 2];
